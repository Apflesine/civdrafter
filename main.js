	...
	for (let player = 1; player <= parseInt(document.getElementById("numberOfPlayers").value); player++) {
		...
		for (let i = 1; i <= parseInt(document.getElementById("numberOfLeaders").value); i++) {
			...
			if (typeof draftedLeader == "undefined") {
				...
				document.getElementById("leadersPlayer"+player).innerHTML += "<img src='https://static.wikia.nocookie.net/civilization/images/"+draftedLeader.img+"' class='leaderIcon'>" + draftedLeader.name + " [" + draftedLeader.civilization + "]<br>[" + draftedMap.name + "]<br>";
			}
		}
	}
}
